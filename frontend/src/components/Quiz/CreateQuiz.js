    ...
    <div className='signup'>
      ...
      {questions.map((q, index) => (
        <><label>Question {`${index+1}`}</label><div key={q.id}>
              <input type="text" value={q.question} placeholder="Write your question here" required="required" onChange={(e) => handleQuestionChange(e, q.id)} />
              <label>Options</label>
              {q.options.map((option, optionIndex) => (
                  <input key={optionIndex} type="text" value={option} required="required" placeholder="Write your choices here" onChange={(e) => handleOptionChange(e, q.id, optionIndex)} />
              ))}
      ...
      {QuizAdded &&  QuizAdded==="Unsuccessful add of Quiz" && <div className='error'>{QuizAdded}</div>}
      {QuizAdded &&  QuizAdded==="Please Write atleast 2 questions" && <div className='error'>{QuizAdded}</div>}
      {QuizAdded && QuizAdded==="Successful add of Quiz" && <div className='success'>{QuizAdded}</div>}
      </form>
    </div>
  );
